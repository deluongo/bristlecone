...
**M0 complete and tagged (`m0`, verified fresh-context 2026-08-20).** M1 build steps (S1–S3) all shipped as of 2026-08-21: validate + render + append-only CI + Pages workflow. What remains of M1: fresh-context verification by the NEXT session (the finishing session may not self-certify), then the operator's public flip (forever-ask).
...
*(none — session M1-S3 complete)*

## Queued (weekend acceleration — operator requested faster cadence through the weekend)

- **M1 fresh-context verification** (next session's first duty): verify M1 against its checklist — validate/render/append-only/pages all live, gates enforced, real archive green — then tag `m1`. The finishing session (M1-S3's) may not self-certify.
- After verification: the public flip is the **operator's button** (forever-ask). At flip time: enable Pages (deploy job arms itself via the `!repository.private` gate; first run may need `workflow_dispatch`), Telegram dead-man bot setup (account-creation forever-ask, S6 pre-granted for the routine itself), domain decision (treasury first claim).
