...
python -m bristlecone lanes                          # validate + list the lane config (lanes.toml)
python -m bristlecone ask records/some-open-record.md            # fan the question out, fill positions
python -m bristlecone ask records/some-open-record.md --dry-run  # whole pipeline, $0, no transport
```

Exit codes: `0` valid, `1` findings, `2` usage/environment error. The renderer is the validator's opposite by design — lenient: a malformed record renders as raw text behind a warning banner, never a crash; broken links are marked, not fatal; and dissent is computed from the front matter and presented in a panel of its own, unedited. CI runs the validator against both the fixture corpus and this repository's real archive, smoke-renders the archive, and enforces append-only semantics on every change: deleting or renaming a record, or editing a frozen one beyond the two permitted fields (`superseded_by` append, `status` → superseded/withdrawn), fails the build. On pushes to main, a Pages workflow renders the site with commit stamps (the deploy arms itself when the repository goes public). M2 is underway: the fan-out substrate has landed — [`lanes.toml`](lanes.toml) declares every model lane as data over exactly two transport kinds (subprocess `cmd` and OpenAI-compatible `openai-http`, per the architecture deliberation below), the loader is fail-closed (an unknown config key is an error, because a typo that silently un-meters a lane is the failure that matters), a scrub filter stands between raw lane output and any tracked file, and the one metered lane refuses `declined:gate` until the operator opens the key-handling gate. The orchestrating `ask` command has now landed on that substrate: it fans an *open* record's question out to the configured lanes and fills attributed positions back in — never creating or deciding a record — with the pipeline order the architecture deliberation pinned (outbound scrub gate that dispatches nothing if it fires, transport, at most one format-repair reprompt, inbound scrub before any byte lands in the record; failed lanes are recorded as failed, never fabricated). `--dry-run` runs that entire pipeline on deterministic fixtures with provably no subprocess, network, or environment access, and never writes the record file. Succession tooling comes in M3. M2's design records are in: the metered-lane key-handling design ([deliberation](records/2026-08-21-key-handling-design.md), where a round-1 dissent reshaped the outcome, and the resulting [`docs/KEY-HANDLING.md`](docs/KEY-HANDLING.md) — a draft awaiting the operator's approval, which is what gates the project's first metered API call) and the fan-out architecture ([deliberation](records/2026-08-21-ask-lane-architecture.md)). Governance is in [`CONSTITUTION.md`](CONSTITUTION.md).
